...
class LMStudioAPI:
    def __init__(self, 
        ...
        self.client = OpenAI(
            base_url=server + "/v1",
            api_key=config.DEFAULT_LMSTUDIO_API_KEY
        )
    ...
    def get_single_model(self, model_id: str) -> dict | None:
        ...
        if self.openai_api:
            logger.warning("Fetching single model details is best via native LM Studio endpoint. OpenAI API might not provide the same level of detail for non-OpenAI models.")
        ...
        logger.debug(f"Fetching single model '{model_id}' from native endpoint: {api_endpoint}")
        try:
            response = requests.get(api_endpoint, timeout=10)
            response.raise_for_status()
    ...
    def call_chat_completions(self, prompt: str, model: str = config.DEFAULT_LMSTUDIO_CHAT_MODEL) -> str | None:
        """
        ...
        if self.openai_api:
            return self.get_chat_completion_openai(prompt=None, model=model) # prompt=None because it's already in history

        # Native LM Studio chat completions
        api_endpoint = self.server + "/api/v0/chat/completions"
        logger.debug(f"Calling native chat completions at: {api_endpoint} with model: {model}")
        ...
        try:
            ...
            logger.exception(f"An error occurred in call_chat_completions (native)")
    ...
    def completions(self, prompt: str, model: str = config.DEFAULT_LMSTUDIO_COMPLETION_MODEL) -> str | None:
        """
        ...
        if self.openai_api:
            ...
            try:
                ...
                chat_completion = self.client.chat.completions.create(
                    model=model,
                    messages=[{"role": "user", "content": prompt}], # Or self.session_history if model handles it well
                    temperature=0.7,
                    max_tokens=150
                )
                ...
                logger.exception(f"An error occurred in completions (OpenAI compatible)")
                if self.session_history and self.session_history[-1]["role"] == "user":
                     self.session_history.pop()
                return None

        # Native LM Studio completions
        self.session_history.append({"role": "user", "content": prompt}) # Add to history before call
        api_endpoint = self.server + "/api/v0/completions"
        logger.debug(f"Calling native completions at: {api_endpoint} with model: {model}")
        headers = {'Content-Type': 'application/json'}
        payload = {
            "model": model,
            "prompt": prompt, 
        ...
        try:
            ...
            logger.exception(f"An error occurred in completions (native)")
            if self.session_history and self.session_history[-1]["role"] == "user":
                self.session_history.pop()
            return None

    def embeddings(self, input_text: str = "Some text to embed", model: str = config.DEFAULT_LMSTUDIO_EMBEDDING_MODEL) -> list | None:
        """
        ...
        if self.openai_api:
            ...
            try:
                ...
                logger.info(f"LMStudio OpenAI Embedding Vector (first 5): {embedding_vector[:5]}...")
                return embedding_vector
            except Exception as e:
                logger.exception(f"An error occurred in embeddings (OpenAI compatible)")
                return None

        # Native LM Studio embeddings
        api_endpoint = self.server + "/api/v0/embeddings"
        logger.debug(f"Requesting native embeddings from: {api_endpoint} for model: {model}")
        headers = {'Content-Type': 'application/json'}
        ...
        try:
            ...
            logger.info(f"LMStudio Native Embedding Vector (first 5): {embedding_vector[:5]}...")
            return embedding_vector
        except Exception as e:
            logger.exception(f"An error occurred in embeddings (native)")
            return None
    ...
    def get_chat_completion_openai(self, prompt: str | None, model: str = config.DEFAULT_LMSTUDIO_CHAT_MODEL) -> str | None:
        """
        ...
        try:
            ...
            logger.exception(f"An error occurred in get_chat_completion_openai")
            # If a prompt was added for this call and it failed, remove it
            if prompt and self.session_history and self.session_history[-1]["role"] == "user":
                self.session_history.pop()
            return None
