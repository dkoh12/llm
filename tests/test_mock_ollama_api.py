...
class TestOllamaAPI(unittest.TestCase):
    ...
    @patch('ollama_api.ollama.chat') # Path to ollama.chat within your ollama_api module
    def test_ollama_chat(self, mock_ollama_chat):
        ...
        expected_messages = self.initial_history + [{"role": "user", "content": test_prompt}]
        mock_ollama_chat.assert_called_once_with(model="llama3.2", messages=expected_messages)
        
        # Assert session_history is updated
        expected_history_after_call = expected_messages + [{"role": "assistant", "content": mock_response_content}]
    ...
    def test_text_completion(self, mock_ollama_generate):
        ...
        mock_ollama_generate.assert_called_once_with(model="codellama:latest", prompt=test_prompt)
    ...
    def test_openai_chat(self, mock_openai_client):
        ...
        mock_openai_client.chat.completions.create.return_value = MockChatCompletion(mock_response_content)
    ...
    def test_get_chat_completion_openai(self, mock_openai_client):
        ...
        expected_messages_call = self.initial_history + [{"role": "user", "content": test_prompt}]
        mock_openai_client.chat.completions.create.assert_called_once_with(
            model="llama3.2:latest",
        ...
        expected_history_after_call = expected_messages_call + [{"role": "assistant", "content": mock_response_content}]
        self.assertEqual(self.api.session_history, expected_history_after_call)
